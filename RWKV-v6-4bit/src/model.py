...
from .module.TimeMix import RWKV6_0_TimeMix
...
class Block(nn.Module):
    ...
    def __init__(self, layer_id, n_layer, n_embd, n_head, head_size, dropout, dim_att, dim_ffn,lora_r, lora_alpha, lora_dropout, lora_quant,lora_quant_type):
        #global LORA_CONFIG
        #lora_r, lora_alpha, lora_dropout = LORA_CONFIG["r"], LORA_CONFIG["alpha"], LORA_CONFIG["dropout"]
        super().__init__()
        ...
        self.att = RWKV6_0_TimeMix(layer_id, n_layer, n_embd, n_head, head_size, dropout, dim_att , lora_r, lora_alpha, lora_dropout,lora_quant,lora_quant_type)
        self.ffn = RWKV_ChannelMix(layer_id, n_layer, n_embd, dim_ffn , lora_r, lora_alpha, lora_dropout,lora_quant,lora_quant_type)
...
class RWKV(L.LightningModule):
    ...
    def __init__(self,
                 ...
                 #if enabled lora_quant will use NF4 Quant via Bitsandbytes
        ...
        print('##################################################')
        print('')
        print(f'Train lora_r={lora_r} lora_alpha={lora_alpha}')
        print('')
        print('##################################################')
        ...
        self.blocks = nn.ModuleList([
            Block(i, n_layer, n_embd, n_head, head_size, dropout, dim_att, dim_ffn,lora_r,lora_alpha,lora_dropout, lora_quant=lora_quant,lora_quant_type=lora_quant_type) for i in range(n_layer)
        ])
        ...
        enable_ln_finetune = 1
        enable_time_finetune = 1
        ...
        if model_weights != None:
            self.load_state_dict(model_weights,strict=False) #if LoRA enabled, missing LoRAKeys
            del model_weights
            gc.collect()
            if lora_quant:
                for name, m in self.named_modules():
                    if hasattr(m, "quant") and callable(getattr(m, "quant")):
                        m.quant(lora_quant_type)
                        print(f'Quant {name}')
        #self.requires_grad_(False)
    ...
    def configure_optimizers(self):
        ...
        if self.deepspeed_offload:
            optimizer = DeepSpeedCPUAdam(optim_groups,
                                         ...
                                         amsgrad=False)
            #optimizer = Lion(optim_groups, lr=lr_init, betas=(self.beta1, self.beta2), weight_decay=self.weight_decay, use_triton=True)
            #print('RAION MODE GAOOOOOOOOOOOOOOOOOO')
        else:
