...
PYTORCH_CLI_ARGV = remove_arg(PYTORCH_CLI_ARGV, "-lora_r")
PYTORCH_CLI_ARGV = remove_arg(PYTORCH_CLI_ARGV, "-lora_alpha")
PYTORCH_CLI_ARGV = remove_arg(PYTORCH_CLI_ARGV, "-lora_dropout")
# ---
...
#        # エポックの終わりに特定のパラメータを保存する
#        if trainer.current_epoch % self.every_n_epochs == 0:  # every_n_epochsはModelCheckpointの引数
#            filepath = f"{self.dirpath}/custom-{trainer.current_epoch}.pth"
#            custom_state_dict = {k: v for k, v in pl_module.state_dict().items() if 'layer1' in k}
#            torch.save(custom_state_dict, filepath)
#            print(f"Saved custom checkpoint to {filepath}")
